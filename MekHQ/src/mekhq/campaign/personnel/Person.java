    ...
    public static Person generateInstanceFromXML(Node wn, Campaign c, Version version) {
        ...
        try {
            ...
            for (String gunSkill : SkillType.rpgGunnerySkillList) {
                if (retVal.skills.keySet().contains(gunSkill)) {
                    Skill skill = retVal.skills.get(gunSkill);
                    for (String rpgGunType : SkillType.rpgGunneryTypeList) {
                        if (!retVal.skills.keySet().contains(SkillType.getRPGSkillName(gunSkill, rpgGunType))) {
